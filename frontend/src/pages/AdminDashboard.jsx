...
const StatsCards = ({ stats }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div className="bg-white/80 backdrop-blur-md rounded-2xl p-6 text-slate-800 border border-slate-200/60 hover:border-blue-500/40 transition-all duration-300 shadow-sm group">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-slate-500 text-sm font-medium">Total Files</p>
          <p className="text-3xl font-bold mt-2 text-slate-800">{stats.totalFiles}</p>
        </div>
        <div className="bg-blue-500/10 p-3.5 rounded-xl border border-blue-500/20 text-blue-600 group-hover:scale-110 transition-transform">
    ...
    <div className="bg-white/80 backdrop-blur-md rounded-2xl p-6 text-slate-800 border border-slate-200/60 hover:border-green-500/40 transition-all duration-300 shadow-sm group">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-slate-500 text-sm font-medium">Total Users</p>
          <p className="text-3xl font-bold mt-2 text-slate-800">{stats.totalUsers}</p>
        </div>
        <div className="bg-green-500/10 p-3.5 rounded-xl border border-green-500/20 text-green-600 group-hover:scale-110 transition-transform">
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="bg-white/80 backdrop-blur-md rounded-2xl p-6 text-slate-800 border border-slate-200/60 hover:border-purple-500/40 transition-all duration-300 shadow-sm group">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-slate-500 text-sm font-medium">Pending Approvals</p>
          <p className="text-3xl font-bold mt-2 text-slate-800">{stats.pendingApprovals}</p>
        </div>
        <div className="bg-purple-500/10 p-3.5 rounded-xl border border-purple-500/20 text-purple-650 group-hover:scale-110 transition-transform">
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const AdminDashboard = () => {
  ...
  return (
    <div 
      className="min-h-screen bg-cover bg-center bg-no-repeat bg-fixed relative text-slate-800 pb-16"
      style={{ backgroundImage: `linear-gradient(rgba(255, 255, 255, 0.78), rgba(255, 255, 255, 0.78)), url('/campus_bg.jpg')` }}
    >
      {/* Header */}
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-white/70 border-b border-slate-200/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              <div className="w-11 h-11 rounded-xl shadow-md border border-slate-200 bg-white overflow-hidden transform hover:scale-105 transition-transform duration-300">
                <img 
                  src="/pwa-192x192.png" 
              ...
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-700 via-indigo-600 to-purple-705 bg-clip-text text-transparent">
                  Admin Dashboard
                </h1>
                <p className="text-xs text-slate-500 mt-0.5 font-medium">Zephyronz Security & Records Panel</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-right hidden sm:block">
                <p className="text-xs text-slate-505">Welcome back,</p>
                <p className="font-semibold text-slate-800 text-sm">{user?.name}</p>
                <p className="text-xs text-indigo-600 font-semibold capitalize">{user?.role}</p>
              </div>
              <div className="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-655 rounded-full flex items-center justify-center text-white font-bold shadow-md shadow-indigo-900/15">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        {activeTab === "upload" && (
          <div className="max-w-2xl mx-auto">
            <div className="backdrop-blur-xl bg-white/80 border border-slate-200/65 shadow-sm rounded-3xl overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 px-6 py-5 border-b border-slate-200/50">
                <h2 className="text-xl font-bold text-slate-800">Upload New File</h2>
                <p className="text-slate-550 text-xs mt-1">Add secure documents accessible to authorized users</p>
              </div>
              ...
              <form onSubmit={handleUpload} className="p-6 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3.5 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-slate-200/80 hover:border-indigo-500/60 rounded-2xl p-8 text-center bg-slate-50/50 hover:bg-slate-50 transition-all duration-200 cursor-pointer">
                    <input
                      id="fileInput"
                      type="file"
                ...
                {uploadProgress > 0 && uploadProgress < 100 && (
                  <div className="w-full bg-slate-100 rounded-full h-2">
                    ...
                    ></div>
                    <p className="text-xs text-slate-500 mt-1.5 text-center">{uploadProgress}% uploaded</p>
                  </div>
                )}
        ...
        {activeTab === "files" && (
          <div className="backdrop-blur-xl bg-white/80 border border-slate-200/65 shadow-sm rounded-3xl overflow-hidden">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-5 border-b border-slate-200/50">
              <h2 className="text-xl font-bold text-slate-800">Manage Files</h2>
              <p className="text-slate-550 text-xs mt-1">View, preview, and remove uploaded documents</p>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-slate-50/60 border-b border-slate-200/60">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Uploaded</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {files.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="px-6 py-12 text-center text-slate-500 bg-slate-50/10">
                    ...
                    files.map((file) => (
                      <tr key={file._id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-800">{file.title}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-655">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex space-x-2">
                            <button
                              ...
                              className="text-indigo-600 hover:text-indigo-500 hover:bg-indigo-555/5 p-2 rounded-xl transition-all"
                              title="Preview"
                            >
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button
                              ...
                              className="text-red-500 hover:text-red-650 hover:bg-red-555/5 p-2 rounded-xl transition-all"
                              title="Delete"
                            >
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {activeTab === "users" && (
          <div className="backdrop-blur-xl bg-white/80 border border-slate-200/65 shadow-sm rounded-3xl overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 px-6 py-5 border-b border-slate-200/50">
              <h2 className="text-xl font-bold text-slate-800">Manage Users</h2>
              <p className="text-slate-550 text-xs mt-1">Approve registrations and manage user permissions</p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-100">
                  {users.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="px-6 py-12 text-center text-slate-500 bg-slate-50/10">
                        No users found
                      </td>
                    </tr>
                  ) : (
                          ...
                          <div className="flex space-x-2">
                            {(userItem.status !== "Approved" && !userItem.isApproved) && userItem.role !== "admin" && (
                              <button
                                ...
                                className="bg-emerald-600 text-white px-3 py-1.5 rounded-xl hover:bg-emerald-700 transition-colors text-xs font-semibold shadow-sm"
                              >
                                Approve
                              </button>
                            )}
                            {userItem.status !== "Rejected" && userItem.role !== "admin" && (
                              <button
                                ...
                                className="bg-amber-500 text-white px-3 py-1.5 rounded-xl hover:bg-amber-600 transition-colors text-xs font-semibold shadow-sm"
                              >
                                Reject
                              </button>
                            )}
                            {userItem.role !== "admin" && (
                              <button
                                ...
                                className="bg-rose-500 text-white px-3 py-1.5 rounded-xl hover:bg-rose-600 transition-colors text-xs font-semibold shadow-sm"
                              >
                                Delete
                              </button>
                            )}
        ...
        {/* Activity Logs Tab */}
        {activeTab === "logs" && (
          <div className="backdrop-blur-xl bg-white/80 border border-slate-200/65 shadow-sm rounded-3xl overflow-hidden animate-fadeIn">
            <div className="bg-gradient-to-r from-rose-50 to-red-50 px-6 py-5 border-b border-slate-200/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-xl font-bold text-slate-800">Security & Activity Logs</h2>
                <p className="text-slate-550 text-xs mt-1">Monitor print blocks, developer shortcut triggers, screenshot attempts, and screen blurs</p>
              </div>
              <div className="flex gap-2.5">
                {selectedLogIds.length > 0 && (
                  <button
                    onClick={handleDeleteSelectedLogs}
                    className="bg-rose-600 hover:bg-rose-700 text-white font-semibold py-2 px-4 rounded-xl transition-all shadow-md text-sm cursor-pointer border border-rose-750/30"
                  >
                ...
                {securityLogs.length > 0 && (
                  <button
                    ...
                    className="bg-white hover:bg-slate-50 text-rose-600 font-semibold py-2 px-4 rounded-xl transition-all shadow-sm text-sm border border-slate-200"
                  >
                    Delete All Logs
                  </button>
                )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                  ...
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-12">
                      <input
                        ...
                        className="rounded border-slate-200 text-rose-650 focus:ring-rose-500 cursor-pointer h-4 w-4 bg-white"
                ...
                <tbody className="divide-y divide-slate-100">
                  {securityLogs.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="px-6 py-12 text-center text-slate-500 bg-slate-50/10">
                    ...
                    securityLogs.map((log) => {
                      ...
                      return (
                        <tr key={log._id} className={`hover:bg-slate-50/50 transition-colors ${isSelected ? 'bg-rose-50/20' : ''}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <span className="text-slate-800 block font-semibold">{log.user?.name || "Visitor"}</span>
                            <span className="text-slate-505 text-xs">{log.user?.email || "Anonymous"}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-2.5 py-0.5 text-xs rounded-full font-bold uppercase tracking-wider border ${
                              log.eventType === "screenshot" 
                                ? "bg-rose-500/10 text-rose-650 border-rose-200" 
                                : log.eventType === "unauthorized_action"
                                ? "bg-orange-500/10 text-orange-655 border-orange-200"
                                : log.eventType === "developer_shortcut"
                                ? "bg-amber-500/10 text-amber-655 border-amber-200"
                                : "bg-sky-500/10 text-sky-655 border-sky-200"
                            }`}>
