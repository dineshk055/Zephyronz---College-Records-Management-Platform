    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-800">Document Hub</h1>
                <p className="text-xs text-slate-500 hidden sm:block">Secure Document Management</p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">Here's an overview of your documents</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500 font-medium">PDF Files</p>
                <p className="text-2xl font-bold text-slate-800 mt-1">{getFileTypeCount("pdf")}</p>
              </div>
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-500 font-medium">Images</p>
                <p className="text-2xl font-bold text-slate-800 mt-1">{getFileTypeCount("image")}</p>
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-slate-800 placeholder-slate-400 transition-all"
              />
            </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="mt-4 text-slate-500 text-sm font-medium">Loading documents...</p>
          </div>
        ) : filteredFiles.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-slate-200">
            <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <HiOutlineDocumentText className="w-10 h-10 text-slate-400" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedFiles.map((file) => (
              <div
                key={file._id}
                className="group bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border border-slate-200"
              >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-slate-800 group-hover:text-indigo-600 transition-colors truncate text-sm">
                          {file.title}
                        </h3>
                    ...
                    <button
                      onClick={() => handleViewFile(file)}
                      className="flex items-center gap-2 px-4 py-2 bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white rounded-lg transition-colors font-medium text-sm"
                    >
                      <FiEye className="w-4 h-4" />
                      <span>View</span>
                    </button>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-8">
            <button
              ...
              className="p-2 bg-white rounded-lg border border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 transition-colors"
            >
          ...
          <div className="relative min-h-screen flex items-center justify-center p-4">
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
                ...
                {selectedFile.pagesData && selectedFile.pagesData.length > 0 ? (
                  <div className="flex flex-col items-center gap-6">
                    {selectedFile.pagesData.map((pageData, index) => (
                      <div 
                        key={index} 
                        className="relative bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden max-w-4xl w-full"
                      ...
                      >
                        <img
                          ...
                          draggable="false"
                        />
                        <div className="text-center text-xs text-slate-400 font-medium border-t border-slate-100 pt-2 mt-2 px-4 pb-2">
                    ...
                    {selectedFile.pages.map((pageFile, index) => (
                      ...
                      >
                        {failedImages[`${selectedFile._id}-${index}`] ? (
                          <div className="flex flex-col items-center justify-center p-12 bg-slate-50 text-slate-500 text-center">
                            <FiAlertCircle className="w-8 h-8 text-amber-500 mb-2" />
                  ...
                  <div className="flex justify-center items-center min-h-[400px] bg-white rounded-xl p-4" onContextMenu={(e) => e.preventDefault()}>
                    {failedImages[`${selectedFile._id}-0`] ? (
                      <div className="flex flex-col items-center justify-center p-12 text-slate-500 text-center">
                        ...
                        <h4 className="font-semibold text-slate-700 mb-1">Image Unavailable</h4>
                        <p className="text-sm text-slate-400">The file has been deleted or is unavailable</p>
                      </div>
                    ) : (
                      <img
                  ...
                  <div className="text-center py-16 bg-white rounded-xl">
                    ...
                    {user?.role === "admin" && selectedFile.fileUrl ? (
                      ...
                      <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-3 max-w-md mx-auto text-sm font-medium">
              ...
              <div className="flex justify-between items-center p-4 border-t border-slate-200 bg-white">
                ...
                <button 
                  onClick={closeModal} 
                  className="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition-colors font-medium text-sm"
                >
                  Close
                </button>
              </div>
